...
void randomize_buttons(SimonGame* simon_game) {
  simon_game->buttons = (uint8_t*)realloc(simon_game->buttons, simon_game->level + 1);
  ...
  switch (game->state) {
    ...
    case SIMON_GAME_STATE_SHOW:
      ...
      for (uint8_t i = 0; i < game->level; i++) {
        ...
        tone_buzzer_driver(&memory_learn->buzzer, BUTTON_FREQUENCIES[game->buttons[i]], duration);
        delay(duration);
      }
      game->state = SIMON_GAME_STATE_PLAY;
      break;
    case SIMON_GAME_STATE_PLAY: {
      ...
      int64_t max_reaction_time = 1000; //TODO: needs to depends on level
      ...
      if (memory_learn->buttons) {
        ...
        for (uint8_t i = 0; i < 8; i++) {
          if (memory_learn->buttons & 1 << i) {
            const uint8_t* rgb_color = BUTTON_COLORS[i];
            tone_buzzer_driver(&memory_learn->buzzer, BUTTON_FREQUENCIES[correct_button], 250);
            memory_learn->leds->setPixelColor(i, rgb_color[0], rgb_color[1], rgb_color[2]);
            // Si le boutton appuyé et correct.
            if (i == correct_button) {
              // Si c'est le dernier bouton a appuyé
              if (game->button_index >= game->level) {
